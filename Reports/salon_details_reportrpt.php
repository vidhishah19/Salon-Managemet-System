...
<?php include_once "salon_details_reportrptinfo.php" ?>
<?php
...
$salon_details_report_rpt = NULL; // Initialize page object first

class crsalon_details_report_rpt extends crsalon_details_report {
	...
	// Page object name
	var $PageObjName = 'salon_details_report_rpt';
	...
	function __construct() {
		...
		// Table object (salon_details_report)
		if (!isset($GLOBALS["salon_details_report"])) {
			$GLOBALS["salon_details_report"] = &$this;
			$GLOBALS["Table"] = &$GLOBALS["salon_details_report"];
		}
		...
		if (!defined("EWR_TABLE_NAME"))
			define("EWR_TABLE_NAME", 'salon details report', TRUE);
		...
		$this->FilterOptions->TagClassName = "ewFilterOption fsalon_details_reportrpt";
	}
...
//		$item->Visible = TRUE;
		...
		$item->Body = "<a title=\"" . ewr_HtmlEncode($ReportLanguage->Phrase("ExportToEmail", TRUE)) . "\" data-caption=\"" . ewr_HtmlEncode($ReportLanguage->Phrase("ExportToEmail", TRUE)) . "\" id=\"emf_salon_details_report\" href=\"javascript:void(0);\" onclick=\"ewr_EmailDialogShow({lnk:'emf_salon_details_report',hdr:ewLanguage.Phrase('ExportToEmail'),url:'$url',exportid:'$exportid',el:this});\">" . $ReportLanguage->Phrase("ExportToEmail") . "</a>";
		$item->Visible = FALSE;
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-caption=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-toggle=\"button\" data-form=\"fsalon_details_reportrpt\">" . $ReportLanguage->Phrase("SearchBtn") . "</button>";
		$item->Visible = TRUE;
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"fsalon_details_reportrpt\" href=\"#\">" . $ReportLanguage->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"fsalon_details_reportrpt\" href=\"#\">" . $ReportLanguage->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
	...
	function ProcessAjaxPopup() {
		...
		if (@$_GET["popup"] <> "") {
			...
			if ($popupname == 'salon_details_report_Salon_Name') {
				$bNullValue = FALSE;
				$bEmptyValue = FALSE;
				$sFilter = $this->Filter;
			...
			if ($popupname == 'salon_details_report_Service_Name') {
				$bNullValue = FALSE;
				$bEmptyValue = FALSE;
				$sFilter = $this->Filter;
	...
	function SetupPopup() {
		...
		if (is_array(@$_SESSION["sel_salon_details_report_Salon_Name"])) {
			$this->LoadSelectionFromSession('Salon_Name');
		} elseif (@$_SESSION["sel_salon_details_report_Salon_Name"] == EWR_INIT_VALUE) { // Select all
			$this->Salon_Name->SelectionList = "";
		}

		// Get Service Name selected values
		if (is_array(@$_SESSION["sel_salon_details_report_Service_Name"])) {
			$this->LoadSelectionFromSession('Service_Name');
		} elseif (@$_SESSION["sel_salon_details_report_Service_Name"] == EWR_INIT_VALUE) { // Select all
			$this->Service_Name->SelectionList = "";
		}
	}
	...
	function GetExtendedFilter() {
		...
		if ($bPostBack) {
			...
			if ($this->PopupName == 'salon_details_report_Salon_Name' && $this->PopupValue <> "") {
		...
		// Reset search command
			...
			if ($this->GetDropDownValue($this->Salon_Name)) {
				$bSetupFilter = TRUE;
			} elseif ($this->Salon_Name->DropDownValue <> EWR_INIT_VALUE && !isset($_SESSION['sv_salon_details_report_Salon_Name'])) {
				$bSetupFilter = TRUE;
			}
			...
			if ($this->GetDropDownValue($this->Service_Name)) {
				$bSetupFilter = TRUE;
			} elseif ($this->Service_Name->DropDownValue <> EWR_INIT_VALUE && !isset($_SESSION['sv_salon_details_report_Service_Name'])) {
				$bSetupFilter = TRUE;
			}
		...
		if ($bSetupFilter) {
			...
			$_SESSION['sel_salon_details_report_Salon_Name'] = ($this->Salon_Name->SelectionList == "") ? EWR_INIT_VALUE : $this->Salon_Name->SelectionList;
			...
			$_SESSION['sel_salon_details_report_Service_Name'] = ($this->Service_Name->SelectionList == "") ? EWR_INIT_VALUE : $this->Service_Name->SelectionList;
		}
	...
	function GetSessionDropDownValue(&$fld) {
		$parm = substr($fld->FldVar, 2);
		$this->GetSessionValue($fld->DropDownValue, 'sv_salon_details_report_' . $parm);
		$this->GetSessionValue($fld->SearchOperator, 'so_salon_details_report_' . $parm);
	}

	// Get filter values from session
	function GetSessionFilterValues(&$fld) {
		$parm = substr($fld->FldVar, 2);
		$this->GetSessionValue($fld->SearchValue, 'sv_salon_details_report_' . $parm);
		$this->GetSessionValue($fld->SearchOperator, 'so_salon_details_report_' . $parm);
		$this->GetSessionValue($fld->SearchCondition, 'sc_salon_details_report_' . $parm);
		$this->GetSessionValue($fld->SearchValue2, 'sv2_salon_details_report_' . $parm);
		$this->GetSessionValue($fld->SearchOperator2, 'so2_salon_details_report_' . $parm);
	}
	...
	function SetSessionDropDownValue($sv, $so, $parm) {
		$_SESSION['sv_salon_details_report_' . $parm] = $sv;
		$_SESSION['so_salon_details_report_' . $parm] = $so;
	}

	// Set filter values to session
	function SetSessionFilterValues($sv1, $so1, $sc, $sv2, $so2, $parm) {
		$_SESSION['sv_salon_details_report_' . $parm] = $sv1;
		$_SESSION['so_salon_details_report_' . $parm] = $so1;
		$_SESSION['sc_salon_details_report_' . $parm] = $sc;
		$_SESSION['sv2_salon_details_report_' . $parm] = $sv2;
		$_SESSION['so2_salon_details_report_' . $parm] = $so2;
	}
	...
	function ClearSessionSelection($parm) {
		$_SESSION["sel_salon_details_report_$parm"] = "";
		$_SESSION["rf_salon_details_report_$parm"] = "";
		$_SESSION["rt_salon_details_report_$parm"] = "";
	}

	// Load selection from session
	function LoadSelectionFromSession($parm) {
		$fld = &$this->fields($parm);
		$fld->SelectionList = @$_SESSION["sel_salon_details_report_$parm"];
		$fld->RangeFrom = @$_SESSION["rf_salon_details_report_$parm"];
		$fld->RangeTo = @$_SESSION["rt_salon_details_report_$parm"];
	}
	...
	function RestoreFilterList() {
		...
		if (array_key_exists("sel_Salon_Name", $filter)) {
			...
			$_SESSION["sel_salon_details_report_Salon_Name"] = $sWrk;
		...
		if (!$bRestoreFilter) { // Clear filter
			$this->SetSessionDropDownValue(EWR_INIT_VALUE, "", "Salon_Name");
			$this->Salon_Name->SelectionList = "";
			$_SESSION["sel_salon_details_report_Salon_Name"] = "";
		}
		...
		if (array_key_exists("sel_Service_Name", $filter)) {
			...
			$_SESSION["sel_salon_details_report_Service_Name"] = $sWrk;
		...
		if (!$bRestoreFilter) { // Clear filter
			$this->SetSessionDropDownValue(EWR_INIT_VALUE, "", "Service_Name");
			$this->Service_Name->SelectionList = "";
			$_SESSION["sel_salon_details_report_Service_Name"] = "";
		}
	}
...
// Create page object
if (!isset($salon_details_report_rpt)) $salon_details_report_rpt = new crsalon_details_report_rpt();
if (isset($Page)) $OldPage = $Page;
$Page = &$salon_details_report_rpt;
...
// Create page object
var salon_details_report_rpt = new ewr_Page("salon_details_report_rpt");

// Page properties
salon_details_report_rpt.PageID = "rpt"; // Page ID
var EWR_PAGE_ID = salon_details_report_rpt.PageID;

// Extend page with Chart_Rendering function
salon_details_report_rpt.Chart_Rendering = 
...
salon_details_report_rpt.Chart_Rendered = 
 function(chart, chartid) { // DO NOT CHANGE THIS LINE!
...
// Form object
var CurrentForm = fsalon_details_reportrpt = new ewr_Form("fsalon_details_reportrpt");

// Validate method
fsalon_details_reportrpt.Validate = function() {
...
fsalon_details_reportrpt.Form_CustomValidate = 
...
fsalon_details_reportrpt.ValidateRequired = true; // Uses JavaScript validation
<?php } else { ?>
fsalon_details_reportrpt.ValidateRequired = false; // No JavaScript validation
<?php } ?>

// Use Ajax
fsalon_details_reportrpt.Lists["sv_Salon_Name"] = {"LinkField":"sv_Salon_Name","Ajax":true,"DisplayFields":["sv_Salon_Name","","",""],"ParentFields":[],"FilterFields":[],"Options":[],"Template":""};
fsalon_details_reportrpt.Lists["sv_Service_Name"] = {"LinkField":"sv_Service_Name","Ajax":true,"DisplayFields":["sv_Service_Name","","",""],"ParentFields":[],"FilterFields":[],"Options":[],"Template":""};
</script>
<?php } ?>
<?php if ($Page->Export == "" && !$Page->DrillDown) { ?>
<script type="text/javascript">
...
<?php if ($Page->Export == "" && !$Page->DrillDown) { ?>
<!-- Search form (begin) -->
<form name="fsalon_details_reportrpt" id="fsalon_details_reportrpt" class="form-inline ewForm ewExtFilterForm" action="<?php echo ewr_CurrentPage() ?>">
<?php $SearchPanelClass = ($Page->Filter <> "") ? " in" : " in"; ?>
<div id="fsalon_details_reportrpt_SearchPanel" class="ewSearchPanel collapse<?php echo $SearchPanelClass ?>">
...
<?php
$Page->Salon_Name->LookupSql = "SELECT DISTINCT `Salon Name`, `Salon Name` AS `DispFld` FROM `salon details report`";
$sWhereWrk = "";
...
<?php
$Page->Service_Name->LookupSql = "SELECT DISTINCT `Service Name`, `Service Name` AS `DispFld` FROM `salon details report`";
$sWhereWrk = "";
...
<script type="text/javascript">
fsalon_details_reportrpt.Init();
fsalon_details_reportrpt.FilterList = <?php echo $Page->GetFilterList() ?>;
</script>
<!-- Search form (end) -->
<?php } ?>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="Salon_ID"><div class="salon_details_report_Salon_ID"><span class="ewTableHeaderCaption"><?php echo $Page->Salon_ID->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="Salon_ID">
<?php if ($Page->SortUrl($Page->Salon_ID) == "") { ?>
		<div class="ewTableHeaderBtn salon_details_report_Salon_ID">
			<span class="ewTableHeaderCaption"><?php echo $Page->Salon_ID->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer salon_details_report_Salon_ID" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->Salon_ID) ?>',2);">
			<span class="ewTableHeaderCaption"><?php echo $Page->Salon_ID->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="Salon_Name"><div class="salon_details_report_Salon_Name"><span class="ewTableHeaderCaption"><?php echo $Page->Salon_Name->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="Salon_Name">
<?php if ($Page->SortUrl($Page->Salon_Name) == "") { ?>
		<div class="ewTableHeaderBtn salon_details_report_Salon_Name">
			<span class="ewTableHeaderCaption"><?php echo $Page->Salon_Name->FldCaption() ?></span>
			<a class="ewTableHeaderPopup" title="<?php echo $ReportLanguage->Phrase("Filter"); ?>" onclick="ewr_ShowPopup.call(this, event, 'salon_details_report_Salon_Name', false, '<?php echo $Page->Salon_Name->RangeFrom; ?>', '<?php echo $Page->Salon_Name->RangeTo; ?>');" id="x_Salon_Name<?php echo $Page->Cnt[0][0]; ?>"><span class="icon-filter"></span></a>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer salon_details_report_Salon_Name" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->Salon_Name) ?>',2);">
			<span class="ewTableHeaderCaption"><?php echo $Page->Salon_Name->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="Owner_Name"><div class="salon_details_report_Owner_Name"><span class="ewTableHeaderCaption"><?php echo $Page->Owner_Name->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="Owner_Name">
<?php if ($Page->SortUrl($Page->Owner_Name) == "") { ?>
		<div class="ewTableHeaderBtn salon_details_report_Owner_Name">
			<span class="ewTableHeaderCaption"><?php echo $Page->Owner_Name->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer salon_details_report_Owner_Name" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->Owner_Name) ?>',2);">
			<span class="ewTableHeaderCaption"><?php echo $Page->Owner_Name->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="Service_Name"><div class="salon_details_report_Service_Name"><span class="ewTableHeaderCaption"><?php echo $Page->Service_Name->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="Service_Name">
<?php if ($Page->SortUrl($Page->Service_Name) == "") { ?>
		<div class="ewTableHeaderBtn salon_details_report_Service_Name">
			<span class="ewTableHeaderCaption"><?php echo $Page->Service_Name->FldCaption() ?></span>
			<a class="ewTableHeaderPopup" title="<?php echo $ReportLanguage->Phrase("Filter"); ?>" onclick="ewr_ShowPopup.call(this, event, 'salon_details_report_Service_Name', false, '<?php echo $Page->Service_Name->RangeFrom; ?>', '<?php echo $Page->Service_Name->RangeTo; ?>');" id="x_Service_Name<?php echo $Page->Cnt[0][0]; ?>"><span class="icon-filter"></span></a>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer salon_details_report_Service_Name" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->Service_Name) ?>',2);">
			<span class="ewTableHeaderCaption"><?php echo $Page->Service_Name->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="User_Name"><div class="salon_details_report_User_Name"><span class="ewTableHeaderCaption"><?php echo $Page->User_Name->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="User_Name">
<?php if ($Page->SortUrl($Page->User_Name) == "") { ?>
		<div class="ewTableHeaderBtn salon_details_report_User_Name">
			<span class="ewTableHeaderCaption"><?php echo $Page->User_Name->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer salon_details_report_User_Name" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->User_Name) ?>',2);">
			<span class="ewTableHeaderCaption"><?php echo $Page->User_Name->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="Address"><div class="salon_details_report_Address"><span class="ewTableHeaderCaption"><?php echo $Page->Address->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="Address">
<?php if ($Page->SortUrl($Page->Address) == "") { ?>
		<div class="ewTableHeaderBtn salon_details_report_Address">
			<span class="ewTableHeaderCaption"><?php echo $Page->Address->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer salon_details_report_Address" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->Address) ?>',2);">
			<span class="ewTableHeaderCaption"><?php echo $Page->Address->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="Price"><div class="salon_details_report_Price"><span class="ewTableHeaderCaption"><?php echo $Page->Price->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="Price">
<?php if ($Page->SortUrl($Page->Price) == "") { ?>
		<div class="ewTableHeaderBtn salon_details_report_Price">
			<span class="ewTableHeaderCaption"><?php echo $Page->Price->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer salon_details_report_Price" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->Price) ?>',2);">
			<span class="ewTableHeaderCaption"><?php echo $Page->Price->FldCaption() ?></span>
...
<span data-class="tpx<?php echo $Page->RecCount ?>_<?php echo $Page->RecCount ?>_salon_details_report_Salon_ID"<?php echo $Page->Salon_ID->ViewAttributes() ?>><?php echo $Page->Salon_ID->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->Salon_Name->Visible) { ?>
		<td data-field="Salon_Name"<?php echo $Page->Salon_Name->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->RecCount ?>_<?php echo $Page->RecCount ?>_salon_details_report_Salon_Name"<?php echo $Page->Salon_Name->ViewAttributes() ?>><?php echo $Page->Salon_Name->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->Owner_Name->Visible) { ?>
		<td data-field="Owner_Name"<?php echo $Page->Owner_Name->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->RecCount ?>_<?php echo $Page->RecCount ?>_salon_details_report_Owner_Name"<?php echo $Page->Owner_Name->ViewAttributes() ?>><?php echo $Page->Owner_Name->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->Service_Name->Visible) { ?>
		<td data-field="Service_Name"<?php echo $Page->Service_Name->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->RecCount ?>_<?php echo $Page->RecCount ?>_salon_details_report_Service_Name"<?php echo $Page->Service_Name->ViewAttributes() ?>><?php echo $Page->Service_Name->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->User_Name->Visible) { ?>
		<td data-field="User_Name"<?php echo $Page->User_Name->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->RecCount ?>_<?php echo $Page->RecCount ?>_salon_details_report_User_Name"<?php echo $Page->User_Name->ViewAttributes() ?>><?php echo $Page->User_Name->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->Address->Visible) { ?>
		<td data-field="Address"<?php echo $Page->Address->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->RecCount ?>_<?php echo $Page->RecCount ?>_salon_details_report_Address"<?php echo $Page->Address->ViewAttributes() ?>><?php echo $Page->Address->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->Price->Visible) { ?>
		<td data-field="Price"<?php echo $Page->Price->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->RecCount ?>_<?php echo $Page->RecCount ?>_salon_details_report_Price"<?php echo $Page->Price->ViewAttributes() ?>><?php echo $Page->Price->ListViewValue() ?></span></td>
<?php } ?>
	</tr>
<?php
...
<?php include "salon_details_reportrptpager.php" ?>
<div class="clearfix"></div>
</div>
<?php } ?>
<?php if ($Page->Export <> "pdf") { ?>
