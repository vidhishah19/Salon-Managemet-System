...
// Functions for PHP Report Maker 9
// (C) 2007-2015 e.World Technology Limited
// Auto load class
...
function CurrentProjectID() {
	...
	return "{3C488A29-896C-4371-BB76-02105C3C2105}";
}
...
function ewr_FilterDropDownValue($fld) {
	...
	if (is_array($fld->DropDownValue)) {
		return implode(", ", $fld->DropDownValue);
	} else {
		return ($fld->DropDownValue == EWR_INIT_VALUE) ? $ReportLanguage->Phrase("PleaseSelect") : $fld->DropDownValue;
	}
}
...
function ewr_ConnectDb(&$info) {
	...
	if ($dbtype == "MYSQL" || $dbtype == "POSTGRESQL" || $dbtype == "ORACLE") {
		...
		// Set date format
		if ($dbtype == "MSSQL" && EWR_DEFAULT_DATE_FORMAT <> "")
...
include_once($EWR_RELATIVE_PATH . "phpmailer5214/PHPMailerAutoload.php");
...
function ewr_GetFileUploadUrl($fld, $val, $resize = FALSE, $encrypt = EWR_ENCRYPT_FILE_PATH, $urlencode = TRUE) {
	if (!ewr_EmptyStr($val)) {
		...
		if ($encrypt) {
			...
			$fn = "ewrfile9.php?t=" . ewr_Encrypt($fld->TblName, $key) ."&fn=" . ewr_Encrypt($path . $val, $key);
			if ($resize)
				$fn .= "&width=" . $fld->ImageWidth . "&height=" . $fld->ImageHeight;
		} elseif ($resize) {
			$fn = "ewrfile9.php?t=" . rawurlencode($fld->TblName) . "&fn=" . ewr_UrlEncodeFilePath($path . $val) .
	...
	if (!ewr_EmptyStr($val)) {
		...
		foreach ($wrkfiles as $wrkfile) {
			...
			if ($bShowImage) {
				...
				$image = ($fn <> "") ? "<a href=\"" . $fn . "\">" . $name . "</a>" : $name;
			}
			if ($bMultiple)
				$images .= "<li>" . $image . "</li>";
			else
