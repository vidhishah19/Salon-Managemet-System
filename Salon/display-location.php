...
if(isset($_GET['did']))
{
    ...
    $deleteq = mysqli_query($connection, "delete from tbl_location where location_id='{$did}'") or die(mysqli_error($connection));
...
FROM
    ...
    INNER JOIN `tbl_location` 
        ON (`tbl_salon`.`salon_id` = `tbl_location`.`salon_id`)
ORDER BY `tbl_location`.`location_id` ASC;") or die(mysqli_error($connection));
$count= mysqli_num_rows($selectq);
echo $count . " Record(s) Found";
while($planrow= mysqli_fetch_array($selectq))
{
    echo "<tr>";
    echo "<td>{$planrow['location_id']}</td>";
    echo "<td>{$planrow['salon_name']}</td>";
    echo "<td>{$planrow['area']}</td>";
    echo "</tr>";
}
?>
<!--
